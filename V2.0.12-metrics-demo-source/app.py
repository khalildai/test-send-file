...
def init_db() -> None:
    with connect() as db:
        ...
        # demo：owner/biz 为固定角色行（共享角色密码，无用户名）
...
def maturity_metric_results(state: dict) -> dict:
    ...
    def team_score(team: dict, domain: str, dimension: str):
        ...
        for level, weight in METRIC_WEIGHTS.items():
            ...
            achieved = sum(1 for row in level_rows if int(row.get("achieved") or 0) == 1)
            total = len(level_rows)
            contribution = achieved / total * weight if total else 0.0
            levels[level] = {"achieved": achieved, "total": total, "weight": weight, "contribution": round(contribution, 4)}
            score += contribution
        ...
        for domain in team.get("domains") or []:
            for dimension in dimensions:
                ...
                if result is not None:
                    team_metrics.append({
                        "scope": "team", "entityId": team.get("id"), "entity": team.get("name"),
                        "domain": domain, "dimension": dimension, **result,
                    })
    ...
    def aggregate_units(kind: str, units: list):
        ...
        for unit in units:
            ...
            for domain, dimension in sorted(pairs):
                samples = [item for item in team_metrics if item["entity"] in names and item["domain"] == domain and item["dimension"] == dimension]
                if samples:
                    output.append({
                        "scope": kind, "entityId": unit.get("id"), "entity": unit.get("name"),
                        "domain": domain, "dimension": dimension,
                        "score": round(sum(item["score"] for item in samples) / len(samples), 4),
                        "teamCount": len(samples), "teams": [item["entity"] for item in samples],
                    })
        return output
    ...
    def averages(scope: str, metrics: list):
        ...
        for domain, dimension in sorted(pairs):
            ...
            output.append({
                "scope": scope, "domain": domain, "dimension": dimension,
                "score": round(sum(item["score"] for item in samples) / len(samples), 4),
    ...
    return {
        "target": 3.0, "maximum": 4.0,
        "formula": "2级达成率×2 + 3级达成率×1 + 4级达成率×1",
        "team": team_metrics,
...
def health():
    with connect() as db:
        revision = current_revision(db)
        user = current_user(db)
        return jsonify({"ok": True, "version": "V2.0.12m1", "revision": revision, "user": user})
