...
DEMO_DELETE_TEAM_NAMES = {"四关节测试组", "六关节测试组"}
    ...
    with WRITE_LOCK:
        with connect() as db:
            ...
            for cap_id, payload in parsed:
                name = str((payload or {}).get("team") or "").strip()
                if name in DEMO_DELETE_TEAM_NAMES:
                    deleted += 1
                    continue
                kept.append((cap_id, payload))
            changed = bind_capability_team_ids([payload for _, payload in kept], teams, departments, businesses)
            if not changed and not deleted:
                return
            db.execute("BEGIN IMMEDIATE")
            if deleted:
                keep_ids = {cap_id for cap_id, _ in kept}
                for cap_id, _ in parsed:
                    if cap_id not in keep_ids:
...
def health():
    with connect() as db:
        revision = current_revision(db)
        user = current_user(db)
    return jsonify({"ok": True, "version": "V2.0.27-demo", "revision": revision, "user": user})
