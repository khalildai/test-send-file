...
<h2>二、总体得分构成、基准与趋势</h2><section class="panel"><p>当前实际得分按 V2.0.14 事实计算；年度目标固定为 3.0；预测得分仅在存在计划月份时计算。当前数据库无完整历史月度快照，历史月份实际值标记为“待接入”，不虚构趋势。</p><div class="two"><div><h3>领域平均得分（柱形图）</h3>{chart_html}</div><div><h3>总体能力达成（饼图）</h3><div class="pie" style="{pie_style}"></div><div class="legend">已达成 {overall["done"]} 项 / {overall["n"]} 项（{overall["rate"]}%）</div><h3>领域达成率</h3>{''.join(f'<span class="tag">{esc(x["name"])} {x["rate"]}%</span>' for x in domains)}</div></div><h3>月度趋势图</h3><div class="trend-empty">历史月度实际快照待接入；当前仅展示当前实际得分，预测值将在计划月份数据齐备后生成。</div></section>
<h2>三、总体得分的部门分解与改进计划</h2><section class="panel"><table><thead><tr><th>部门</th><th>当前得分</th><th>达成率</th><th>能力项数</th><th>目标差距</th><th>改进计划</th></tr></thead><tbody>{dept_rows}</tbody></table></section>
<h2>四、总体得分的业务分解与改进计划</h2><section class="panel"><table><thead><tr><th>业务线</th><th>当前得分</th><th>达成率</th><th>能力项数</th><th>目标差距</th><th>改进计划</th></tr></thead><tbody>{business_rows}</tbody></table></section>
