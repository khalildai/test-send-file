...
valid_domains = [x for x in domains if x['score'] is not None]
best_domain = max(valid_domains, key=lambda x: x['score']) if valid_domains else None
weak_domain = min(valid_domains, key=lambda x: x['score']) if valid_domains else None
valid_depts = [x for x in group('dept') if x['score'] is not None]
valid_business = [x for x in business if x['score'] is not None]
overall_note = ('当前总体得分为 %.2f，较年度目标 3.0 低 %.2f 分，达成率 %.1f%%；建议优先围绕低分领域和未达成能力制定改进计划。' % (overall['score'], 3-overall['score'], overall['rate'])) if overall['score'] is not None and overall['score'] < 3 else ('当前总体得分达到年度目标 3.0，达成率 %.1f%%；后续重点转向保持高分领域并巩固短板。' % overall['rate'])
domain_note = ('领域分析显示，%s 得分最高（%.2f），%s 得分最低（%.2f），两者相差 %.2f 分。' % (best_domain['name'], best_domain['score'], weak_domain['name'], weak_domain['score'], best_domain['score']-weak_domain['score'])) if best_domain and weak_domain else '当前领域暂无可比较的成熟度得分。'
dept_note = ('部门分析显示，当前最高部门为 %s（%.2f），最低部门为 %s（%.2f）；建议优先核查最低部门的未达成能力。' % (max(valid_depts,key=lambda x:x['score'])['name'], max(valid_depts,key=lambda x:x['score'])['score'], min(valid_depts,key=lambda x:x['score'])['name'], min(valid_depts,key=lambda x:x['score'])['score'])) if valid_depts else '当前暂无可分析的部门得分。'
business_note = ('业务分析显示，当前最高业务线为 %s（%.2f），最低业务线为 %s（%.2f）；业务改进计划应直接关联其领域×评估维度短板。' % (max(valid_business,key=lambda x:x['score'])['name'], max(valid_business,key=lambda x:x['score'])['score'], min(valid_business,key=lambda x:x['score'])['name'], min(valid_business,key=lambda x:x['score'])['score'])) if valid_business else '当前暂无可分析的业务线得分。'
...
def table_rows(items, label):
    result=[]
    for x in items:
        gap = '—' if x['score'] is None else format(x['score'] - 3, '.2f')
...
<h2>一、总体成熟度达成概览</h2><div class="grid"><div class="card">总体实际得分<b>{overall['score'] if overall['score'] is not None else '—'}</b><span class="muted">目标 3.0</span></div><div class="card">能力达成率<b>{overall['rate']}%</b><span class="muted">{overall['done']} / {overall['n']} 项</span></div><div class="card">距目标差距<b>{('+' if overall['score'] and overall['score']>=3 else '') + ('—' if overall['score'] is None else f"{overall['score']-3:.2f}")}</b><span class="muted">实际得分 - 3.0</span></div><div class="card">未达成能力<b>{overall['n']-overall['done']}</b><span class="muted">待改进范围</span></div></div><section class="panel"><b>数据分析：</b>{overall_note}</section>
<h2>二、总体得分构成、基准与趋势</h2><section class="panel"><p>当前实际得分按 V2.0.14 事实计算；年度目标固定为 3.0；预测得分仅在存在计划月份时计算。当前数据库无完整历史月度快照，历史月份实际值标记为“待接入”，不虚构趋势。</p><div class="two"><div><h3>领域平均得分（柱形图）</h3>{chart_html}</div><div><h3>总体能力达成（饼图）</h3><div class="pie" style="{pie_style}"></div><div class="legend">已达成 {overall["done"]} 项 / {overall["n"]} 项（{overall["rate"]}%）</div><h3>领域达成率</h3>{''.join(f'<span class="tag">{esc(x["name"])} {x["rate"]}%</span>' for x in domains)}</div></div><p><b>数据分析：</b>{domain_note}</p><h3>月度趋势图</h3><div class="trend-empty">历史月度实际快照待接入；当前仅展示当前实际得分，预测值将在计划月份数据齐备后生成。</div></section>
<h2>三、总体得分的部门分解与改进计划</h2><section class="panel"><p><b>数据分析：</b>{dept_note}</p><table><thead><tr><th>部门</th><th>当前得分</th><th>达成率</th><th>能力项数</th><th>目标差距</th><th>改进计划</th></tr></thead><tbody>{dept_rows}</tbody></table></section>
<h2>四、总体得分的业务分解与改进计划</h2><section class="panel"><p><b>数据分析：</b>{business_note}</p><table><thead><tr><th>业务线</th><th>当前得分</th><th>达成率</th><th>能力项数</th><th>目标差距</th><th>改进计划</th></tr></thead><tbody>{business_rows}</tbody></table></section>
