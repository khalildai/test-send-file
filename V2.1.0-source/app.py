...
def init_db() -> None:
    with connect() as db:
        ...
        audit_cols = {row[1] for row in db.execute("PRAGMA table_info(audit_logs)")}
        if "ip" not in audit_cols:
            db.execute("ALTER TABLE audit_logs ADD COLUMN ip TEXT")
        if "source" not in audit_cols:
            db.execute("ALTER TABLE audit_logs ADD COLUMN source TEXT")
...
def migrate_capability_team_ids() -> None:
    with WRITE_LOCK:
        with connect() as db:
            ...
            delete_ids = [
                cap_id for cap_id, payload in parsed
                if str(payload.get("team") or "").strip() in TEAMS_TO_DELETE
            ]
            kept = [(cap_id, payload) for cap_id, payload in parsed if cap_id not in set(delete_ids)]
            changed = bind_capability_team_ids([payload for _, payload in kept], teams, departments, businesses)
            if not changed and not delete_ids:
                return
            db.execute("BEGIN IMMEDIATE")
            for cap_id in delete_ids:
                db.execute("DELETE FROM capabilities WHERE id=?", (cap_id,))
            for cap_id, payload in kept:
...
def replace_state(db: sqlite3.Connection, payload: dict, new_revision: int) -> None:
    ...
    for index, row in enumerate(payload.get("data") or []):
        cap_id = str(row.get("id") or f"cap-{index + 1}")
        row["id"] = cap_id
        unique_caps[cap_id] = row
    db.executemany(
        "INSERT INTO capabilities(id, payload) VALUES (?, ?)",
        [(cap_id, json_value(row)) for cap_id, row in unique_caps.items()],
    )
    db.executemany(
...
def apply_server_timestamps(old_rows: list, new_rows: list) -> None:
    ...
    for row in new_rows:
        ...
        def _as_achieved(value):
            if value in (None, ""):
                return None
            return 1 if int(value) else 0
        old_achieved = _as_achieved(previous.get("achieved"))
        new_achieved = _as_achieved(row.get("achieved"))
        row["achieved"] = new_achieved
        row["achievedAt"] = (
...
def canonical_org_config(config) -> dict:
    config = config or {}

    def as_id_list(items):
        return sorted(str(item) for item in (items or []) if str(item).strip())
    ...
    for team in config.get("teams") or []:
        teams.append({
            ...
            "domains": sorted(str(domain) for domain in (team.get("domains") or []) if str(domain).strip()),
        })
    teams.sort(key=lambda item: item["id"])

    def units(kind: str):
        rows = []
        for unit in config.get(kind) or []:
            rows.append({
                "id": str(unit.get("id") or ""),
                "name": str(unit.get("name") or "").strip(),
                "teamIds": as_id_list(unit.get("teamIds") or []),
            })
    ...
    return {
        ...
        "businesses": units("businesses"),
        "retiredTeamIds": as_id_list(config.get("retiredTeamIds") or []),
        "retiredDeptIds": as_id_list(config.get("retiredDeptIds") or []),
        "retiredBusinessIds": as_id_list(config.get("retiredBusinessIds") or []),
    }
...
def health():
    with connect() as db:
        revision = current_revision(db)
        user = current_user(db)
    return jsonify({"ok": True, "version": "V2.0.29d18", "revision": revision, "user": user})
...
def put_state():
    ...
    if user["role"] != "admin":
        with connect() as db:
            ...
            old_config = load_state(db).get("configData") or {}
        if payload.get("auditSource") == "excel":
            return jsonify({"error": "仅管理员可导入 Excel"}), 403
        old_ids = {str(r.get("id")) for r in old_rows if r.get("id")}
        new_ids = {str(r.get("id")) for r in (payload.get("data") or []) if r.get("id")}
        if old_ids - new_ids:
            return jsonify({"error": "仅管理员可删除能力项"}), 403
        if canonical_org_config(old_config) != canonical_org_config(payload.get("configData")):
            return jsonify({"error": "仅管理员可修改组织关系配置"}), 403
        try:
    ...
    with WRITE_LOCK:
        with connect() as db:
            ...
            if audit_entries:
                ...
                source = payload.get("auditSource")
                source = "导入 Excel" if source == "excel" else "页面修改"
                db.executemany(
                    "INSERT INTO audit_logs(ts, operator, role, target, field, old_value, new_value, ip, source) VALUES (?,?,?,?,?,?,?,?,?)",
                    [(ts, user.get("displayName") or user["username"], user["role"], t, f, o, n, ip, source) for (t, f, o, n) in audit_entries],
                )
            db.commit()
...
def request_client_ip() -> str:
    """改数据请求的来源 IP：优先反向代理 X-Forwarded-For 第一段，否则 remote_addr。不用于纯浏览。"""
...
def get_audit_logs():
    with connect() as db:
        rows = db.execute(
            "SELECT seq, ts, operator, role, target, field, old_value, new_value, ip, source FROM audit_logs ORDER BY seq DESC LIMIT 500"
        ).fetchall()
    ...
    return jsonify({"logs": [
        {"seq": r["seq"], "time": r["ts"], "operator": r["operator"], "role": role_label.get(r["role"], r["role"]),
         ...
         "ip": r["ip"] or "", "source": (r["source"] or "页面修改")}
        for r in rows
    ]})
