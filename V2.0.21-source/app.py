...
def health():
    with connect() as db:
        revision = current_revision(db)
        user = current_user(db)
    return jsonify({"ok": True, "version": "V2.0.21", "revision": revision, "user": user})
    ...
    with WRITE_LOCK:
        with connect() as db:
            ...
            if audit_entries:
                ...
                db.executemany(
                    "INSERT INTO audit_logs(ts, operator, role, target, field, old_value, new_value, ip) VALUES (?,?,?,?,?,?,?,?)",
                    [(ts, user.get("displayName") or user["username"], user["role"], t, f, o, n, ip) for (t, f, o, n) in audit_entries],
                )
            db.commit()
...
def get_audit_logs():
    with connect() as db:
        rows = db.execute(
            "SELECT seq, ts, operator, role, target, field, old_value, new_value, ip FROM audit_logs ORDER BY seq DESC LIMIT 500"
        ).fetchall()
