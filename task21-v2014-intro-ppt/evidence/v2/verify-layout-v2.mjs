...
const expression = `(() => {
  ...
  const overflowTargets = all.filter((node) => node.matches('.topline,.matrix,.cell,.bottom,.example-panel,.example-body,.sample,.sample-score,.role-panel,.role-list,.role,.proof,.era,.row-label'));
  ...
  const groups = ['.topline', '.evolution', '.matrix', '.bottom', '.example-body', '.role-list', '.domains', '.standard-flow', '.level-flow', '.proof'];
  const overlaps = [];
