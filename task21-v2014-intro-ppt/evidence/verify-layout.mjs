...
const expression = `(() => {
  ...
  const overflowTargets = all.filter((node) => node.matches('.topline,.matrix,.cell,.bottom,.role-panel,.plan-panel,.role,.plan-item,.proof,.era,.row-label'));
  ...
  const groups = ['.topline', '.evolution', '.matrix', '.bottom', '.role-grid', '.plan-list', '.domains', '.standard-flow', '.level-flow', '.proof'];
  const overlaps = [];
