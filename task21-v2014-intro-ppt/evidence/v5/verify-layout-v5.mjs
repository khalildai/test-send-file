...
const expression = `(() => {
  ...
  const overflowTargets = all.filter((node) => node.matches('.topline,.evolution,.matrix,.cell,.old-map,.old-modules,.domain-graphic,.domain-grid,.icon-flow,.icon-step,.value-visual,.goal-box,.steps,.step,.insight-flow,.insight,.bottom,.data-panel,.demo-body,.demo-card,.team-demo,.radar,.team-insight,.insight-row,.dept-bars,.mini-bar-row,.heat-head,.heat-row,.role-panel,.value-body,.value-role,.era,.row-label'));
  ...
  const groups = ['.topline', '.evolution', '.matrix', '.bottom', '.old-map', '.old-modules', '.domain-grid', '.icon-flow', '.value-visual', '.steps', '.insight-flow', '.demo-body', '.team-demo', '.team-insight', '.dept-bars', '.heat-head', '.heat-row', '.value-body'];
  const overlaps = [];
