...
const expression = `(() => {
  ...
  const overflowTargets = all.filter((node) => node.matches('.topline,.evolution,.matrix,.cell,.domain-graphic,.domain-grid,.icon-flow,.icon-step,.value-visual,.stairs,.insight-flow,.insight,.bottom,.data-panel,.compare-list,.compare-row,.entity,.role-panel,.insight-body,.role-strip,.role-mini,.decision-list,.decision-row,.era,.row-label'));
  ...
  const groups = ['.topline', '.evolution', '.matrix', '.bottom', '.domain-grid', '.icon-flow', '.value-visual', '.stairs', '.insight-flow', '.compare-list', '.compare-row', '.entity', '.role-strip', '.decision-list'];
  const overlaps = [];
