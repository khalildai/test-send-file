...
const expression = `(() => {
  ...
  const overflowTargets = all.filter((node) => node.matches('.topline,.era,.special,.roles,.role,.matrix,.cell,.three,.module,.design-new,.owner-flow,.owner-source,.owner-distribution,.owner-dims,.owner-teams,.domains,.domain-grid,.domain,.target-flow,.target-scenes,.target-scene,.projects-mini,.same-goal,.op,.value-old,.value-new,.goal-box,.steps,.outcomes,.outcome,.example,.example-grid,.panel,.radar-wrap,.radar,.diag-list,.diag-item,.dept-chart,.plot,.bars,.bar,.analysis-line,.domain-bars,.domain-row,.pair,.trend,.advice,.advice-row,.risk'));
  ...
  const groups = ['.topline','.era','.special','.roles','.matrix','.three','.design-new','.owner-flow','.owner-dims','.target-scenes','.projects-mini','.domain-grid','.value-old','.value-new','.steps','.outcomes','.example-grid','.radar-wrap','.diag-list','.bars','.domain-bars','.domain-row','.analysis-line','.advice','.risk'];
  const overlaps = [];
  ...
  const req = {
    ...
    ownerFlow: ['能力 Owner','统一设计评估维度能力','统一分发','各涉域科组','设计','方案','用例','执行','自动化'].every((item) => text.includes(item)),
    targetFlow: ['目标：试点','目标：全面落地','各科组 3.0','部分项目','所有项目'].every((item) => text.includes(item)),
    removedOldDesignCards: !text.includes('能力 Owner 统一提供') && !text.includes('各组目标一致')
  };
  return {
...
const requirementsOk = req.comparisonRatio >= 0.667 && req.comparisonRatio <= 0.8 && req.exampleHeight >= 290 && req.sixDomains === 6 && req.oldOperationIcons === 3 && req.analysisLabels === 2 && req.riskLabels === 1 && req.removedExampleSubtitle && req.hasProgressWording && req.preservedViews && req.ownerFlow && req.targetFlow && req.removedOldDesignCards;
